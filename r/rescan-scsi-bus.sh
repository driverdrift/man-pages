...
NAME
       rescan-scsi-bus.sh  - script to add and remove SCSI devices without re‐
       booting

SYNOPSIS
       rescan-scsi-bus.sh   [--alltargets]   [--attachpq3]   [-c]    [---chan‐
       nels=CLIST]  [--color] [-d] [--flush] [-f] [--forceremove] [--forceres‐
       can]  [--help]  [--hosts=HLIST]  [--ids=TLIST]  [--ignore-rev]   [--is‐
       sue-lip]   [-i]   [--issue-lip-wait=SECS]   [-I  SECS]  [-l]  [-L  NUM]
       [--largelun] [--luns=LLIST] [-m] [--multipath] [--nooptscan] [--nosync]
       [--remove] [--removelun2] [--resize] [--sparselun] [--sync]  [--update]
       [--version] [--wide] [HOST1 [HOST2 ...]]

OPTIONS
       Option  are  ordered  by their long name. Those without a long name are
       ordered as if their single letter was a long name.
       ...
       --ignore-rev
              ignore (firmware) revision change. This is the third text  field
              (4 bytes long) in a standard INQUIRY response.
       ...
       -w, --wide
              ...
              (deprecated) or with the --hosts=LIST parameter (recommended).

       Arguments to options that end in LIST (e.g. TLIST) can have this form:
           A[-B][,C[-D]]...
       which  is  a  comma  separated  list of single values and/or ranges (no
       spaces allowed).

SEE ALSO
       There  is  a  brief  descripion   here:   http://fibrevillage.com/stor‐
       age/585-rescan-scsi-bus-sh-script-for-adding-and-removing-scsi-devices-
       without-rebooting
